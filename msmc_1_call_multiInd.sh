...
#        MASK_IND=${OUTDIR}/mask/ind_mask.${IND}.${s}.${METHOD}.bed.gz # Individual mask file to be created
        ...
        if [ "$METHOD" == "samtools" ]; then
                ...
                bcftools mpileup -Ou -r ${s} --threads 16 -f $GENOME -a INFO/AD --bam-list $BAMLIST | bcftools call -c --threads 16 -V indels | bcftools view -i 'INFO/AD>5' -Ov > ${VCF}
...
### Report:
echo "Filtered VCF and mask created for all scaffolds for ${BAMLIST}."
